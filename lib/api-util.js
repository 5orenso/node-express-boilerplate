...
// Load all api-handlers from "lib/api-handlers/"
let normalizedPath = require('path').join(__dirname, 'api-handlers'),
    fs = require('fs');
 ...
 */
ApiUtil.prototype.parseApiRequest = (req) => {
    //console.log(req);
 ...
 * @returns {*}
 */
ApiUtil.prototype.handleApiRequest = (apiRequest) => {
    let result,
        resource = apiRequest.resource;
    let id = _.isEmpty(apiRequest.related) ? apiRequest.resourceId : apiRequest.relatedId;
    let functionName = apiRequest.method.toLowerCase() +
    ...
    // Dynamically invoke module and function based on api endpoint url.
    // Examples:
    //     GET    /api/articles/123/comments -> ./api-handlers/articles.prototype.getArticlesComments()
    //     GET    /api/articles/123 -> ./api-handlers/articles.prototype.getArticles()
    //     POST   /api/articles/123 -> ./api-handlers/articles.prototype.postArticles()
    //     PUT    /api/articles/123 -> ./api-handlers/articles.prototype.putArticles()
    //     PATCH  /api/articles/123 -> ./api-handlers/articles.prototype.patchArticles()
    //     DELETE /api/articles/123 -> ./api-handlers/articles.prototype.deleteArticles()
    if (typeof apiHandlerModules[resource] === 'object' &&
        typeof apiHandlerModules[resource][functionName] === 'function') {
        // The api handler function must return a promise.
 ...
 * @returns {boolean}
 */
ApiUtil.prototype.sendHeaderResponse = (req, res, opt) => {
    let httpHeaders = _.snapshot(httpHeadersTemplate);
...
ApiUtil.prototype.makeErrorMsg = (opt) => {
    return {
...
ApiUtil.prototype.sendJsonResponse = (req, res, opt) => {
    let customHttpHeaders = {};
    let response = _.snapshot(responseTemplate);
    ...
    let statusJson = JSON.stringify(response, null, 4);
